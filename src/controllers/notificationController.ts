...
const checkNotificationOwnership = async (notificationId: number, userId: number, role?: string) => {
  if (role === 'ADMIN') {
...
export const getNotificationById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const notification = await checkNotificationOwnership(notificationId, userId, role);
